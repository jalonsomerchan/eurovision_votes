...
describe('Eurovision prediction utility', () => {
  ...
  it('keeps prediction UI translated and data-driven', () => {
    ...
    assert.match(app, /data-prediction-preview-image/);
    assert.match(app, /data-prediction-modal/);
    ...
    assert.match(labels, /Generar imagen/);
    assert.match(labels, /shareUnavailable/);
    assert.match(labels, /eurovision\.alon\.one/);
    assert.doesNotMatch(labels, /winnerTitle|winnerLabel|winnerSummary|Ganador previsto/);
  ...
  it('splits prediction client logic into small modules without Firebase', () => {
    ...
    assert.match(entry, /prepareImagePreview/);
    assert.match(entry, /buildPredictionImage/);
    assert.match(entry, /downloadPredictionImage/);
    assert.match(entry, /sharePredictionImage/);
    assert.match(entry, /countryGrid/);
    assert.doesNotMatch(entry, /data-prediction-winner|nodes\.winner|winner:/);
    ...
    assert.doesNotMatch(data, /winner/);
    ...
    assert.match(image, /wrapText/);
    assert.match(image, /bottomPadding/);
    assert.match(image, /labels\.credit/);
    assert.doesNotMatch(image, /winner/);
    ...
    assert.match(docs, /puesto 1 actúa como ganador/);
  });
});
